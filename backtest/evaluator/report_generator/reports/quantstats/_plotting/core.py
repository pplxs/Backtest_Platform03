...
def plot_timeseries(
    ...
    # btc_price.fillna(0, inplace=True)
    ...
    if btc_price is not None:
        # 第二个 y 轴 (BTC价格曲线)
        ax2 = ax.twinx()
        # ax2.plot(btc_price, lw=lw, label='BTCUSDT_SPOT', color=colors[5])
        # import pdb; pdb.set_trace()
        btc_price = btc_price.dropna().reindex(returns.index)
        ax2.plot(btc_price.index, btc_price.values, lw=lw, label='BTCUSDT_SPOT', color=colors[5])

        ax2.set_ylabel("BTC Price", color=colors[5], fontweight="bold")
